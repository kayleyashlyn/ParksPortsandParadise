...
| **Testimonial** | The revolving testimonials section on the homepage | A quote, a client name, and two optional fields: **Trip Label** (short context, e.g. "First Cruise") and **Advisor Name** (which of you handled the trip, if you want it called out). **Display order** controls the rotation position — set it on every new one (1, 2, 3…) or it won't know where to go. **Client Name** defaults to "Verified PPP client" on a new document — leave it that way until the client has said it's okay to use their real name, then edit it in. There's no "active" toggle — to pull a testimonial out of rotation, just delete it. Five real client quotes are ready to paste in (see the schema file's code comment, or ask your developer) — all five keep the "Verified PPP client" placeholder for now. |
...
| 2026-09-14 | §3 — added the **Testimonial** content model for the homepage's revolving testimonials section (quote, client name, optional trip label / advisor name, display order). Five real client quotes confirmed and ready to paste in — see the schema file's code comment. No new account, env var, or integration; UI build to render the section is a separate follow-up. |
