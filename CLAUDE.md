...
- **Styling embargo for `ui-agent` — mostly lifted (logos delivered 2026-09-08):** the brand refresh has now delivered **color values, type families, and logo files** (see `BRAND_KIT.md`). Palette + type are wired into `tailwind.config.ts` and `app/globals.css` (brand hex drives the shadcn CSS variables; Playfair Display + Inter via `next/font`); logos live in `public/images/logos/` (`logo-primary.png`, `logo-white.png`, `logo-black.png`) with a raw `public/favicon.png`. Tailwind design-token work **and** header/hero logo lockups are **unblocked** — build against those assets, not placeholder/current-site branding. **Still outstanding:** brand photography / photography direction — do not ship photography-dependent layouts as "done" against stock stand-ins; flag them for the client instead. (History: this work was fully blocked until the kit arrived, since building against the old brand means a rework, not a head start.)
