...
- **Styling embargo for `ui-agent` — partially lifted (2026-09-08):** the client's brand refresh has delivered **color values and type families** (see `BRAND_KIT.md`), and these are now wired into `tailwind.config.ts` and `app/globals.css` (brand hex drives the shadcn CSS variables; Playfair Display + Inter via `next/font`). Tailwind **design-token work (color system, type scale) is unblocked** — build against those tokens, not placeholder/current-site branding. **Still outstanding:** logo files and brand photography / photography direction. Until those land, do not finalize header/hero logo lockups or ship photography-dependent layouts as "done" against stock stand-ins — flag them for the client instead. (History: token work was fully blocked until the kit arrived, since building against the old brand means a rework, not a head start.)
