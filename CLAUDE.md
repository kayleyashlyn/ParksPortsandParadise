...
- **Build status (2026-09-09, PRs #4–#13 merged; blog in review):** `npm run build` + `npx tsc --noEmit` green (22 pages). Routes: `/` · `/destinations` · `/destinations/[slug]` (SSG) · `/meet-the-team` · `/plan-your-vacation` (dynamic — multi-step RHF + Zod form, reads `?destination=`) · `/work-with-us` (+ `/api/work-with-us`) · `/blog` · `/blog/[slug]` (SSG + ISR, Portable Text body) · `/privacy` · `/terms` (draft — `LEGAL_DRAFT` in `lib/legal.ts`) · `/api/vacation-request` · `/sitemap.xml` · `/robots.txt` · `/studio/[[...tool]]` · icon/favicon routes. GA4 loads via `@next/third-parties`, double-gated on the measurement-ID env var **and** analytics consent (`components/analytics.tsx` + `components/cookie-consent.tsx` + `lib/consent.ts`). Form API routes share `lib/notify.ts`. Deps: `resend`, `@next/third-parties`, `@tailwindcss/typography`, `@portabletext/react`. No routes 404 in nav any more. Keep build + tsc green before every commit — see "Before committing" below.
...
- **Email + analytics are wired but env-gated — the no-ops are deliberate, not bugs.** Both form API routes (`vacation-request`, `work-with-us`) send via Resend through the shared `lib/notify.ts` `sendNotification()`; GA4 loads via `@next/third-parties`. Email stays inert until the client sets `RESEND_API_KEY` (+ a Resend-verified sending domain); GA4 stays inert until `NEXT_PUBLIC_GA4_MEASUREMENT_ID` is set **and** the visitor accepts analytics cookies — see `TODO.md`. Analytics is **opt-in**: `components/cookie-consent.tsx` shows a banner (not for GPC users — a GPC signal is a silent decline), `components/analytics.tsx` renders `<GoogleAnalytics>` only on `consent === "granted"`, and revoking via the footer "Cookie settings" control reloads to tear the tag down. Forms fire their GA event (`generate_lead` / `submit_application`) **only when the API reports delivery** (`window.gtag?.()` — a safe no-op when analytics is off), so a broken Resend or a declining visitor can't inflate conversions. Don't "fix" these to always send / always fire, don't bypass the consent gate, and don't add a second GA script.
